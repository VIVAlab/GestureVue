...
void equalize(const cv::Mat &image, cv::Mat &result){
    ...
    for(int j = 0; j<image.rows; j++){
        ...
        for(int i = 0*image.channels(); i<nc; i=i+image.channels()){
            ...
            *output++= cv::saturate_cast<uchar>((int)(256.0*(((double)(current[i]))/(sqrt(((double)(current[i]*current[i] + current[i+1]*current[i+1] + current[i+2]*current[i+2])))))));
            *output++= cv::saturate_cast<uchar>((int)(256.0*(((double)(current[i+1]))/(sqrt(((double)(current[i]*current[i] + current[i+1]*current[i+1] + current[i+2]*current[i+2])))))));
            *output++= cv::saturate_cast<uchar>((int)(256.0*(((double)(current[i+2]))/(sqrt(((double)(current[i]*current[i] + current[i+1]*current[i+1] + current[i+2]*current[i+2])))))));
            //sharpen_pixel = 5*current - left -right -up -down;
        }
    }
